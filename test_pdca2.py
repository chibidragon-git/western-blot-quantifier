...
def main():
    ...
    # v9: 個別レーンピーク検出 (改良版)
    results_v9 = []
    # ... (omitted for brevity in thinking, will implement in v10)

    # v10: ハイブリッド (個別レーン位置 + v8の積分方式)
    results_v10 = []
    ...
    for i in range(num_lanes):
        ...
        inv = np.maximum(np.percentile(prof, 90) - prof, 0)
        if inv.max() > 5:
            pk = np.argmax(inv)
            t = pk
            while t > 0 and inv[t] > inv[pk]*0.2: t -= 1
            b = pk
            while b < len(inv)-1 and inv[b] > inv[pk]*0.2: b += 1
            all_tops.append(t)
            all_bottoms.append(b)
    
    if all_tops:
        final_top = max(0, min(all_tops) - 5)
        final_bottom = min(h - 1, max(all_bottoms) + 5)
    else:
        final_top, final_bottom = 0, h-1
        
    for i in range(num_lanes):
        x_start = i * lane_width
        x_end = (i + 1) * lane_width if i < num_lanes - 1 else w
        lane = gray[final_top:final_bottom+1, x_start:x_end]
        bg = np.percentile(lane, 95)
        vol = np.sum(np.maximum(bg - lane.astype(np.float64), 0))
        results_v13.append({'Lane': i + 1, 'Volume': vol})
    methods.append(('v13_enveloped_roi', results_v13))
